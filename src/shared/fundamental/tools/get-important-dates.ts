...
/**
 * 註冊股票重要日期查詢工具到 MCP Server
 * @param {McpServer} server MCP Server 實例
 * @param {FugleApiProvider} provider 股票 Fugle API 客戶端
 */
export function registerImportantDatesTool(
  server: McpServer,
  provider: FugleApiProvider
) {
  const currentDir = path.join(__dirname, '..');
  const { description } = loadToolMetadata(currentDir, 'get-important-dates', '獲取股票最近重要日期');
  ...
  server.tool(
    ...
    createToolHandler(
      ...
      'get-important-dates',
      async ({ symbolId }) => {
        const data = await provider.getImportantDates(symbolId);
        return { data };
      },
      {
