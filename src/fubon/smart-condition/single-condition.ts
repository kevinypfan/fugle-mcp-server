...
export function registerSingleConditionTool(
  ...
  server.tool(
    ...
    "建立單一條件單",
    {
      ...
      condition_trigger: z.enum(["BidPrice", "AskPrice", "MatchedPrice", "TotalQuantity", "Time"]).describe(
        "觸發條件：BidPrice = 買價, AskPrice = 賣價, MatchedPrice = 成交價, TotalQuantity = 總量, Time = 時間"
      ),
