...
import realizedPnLSumReference from "./references/realized-pnl-sum.json";
  ...
  server.tool(
    ...
    async ({ startDate, endDate }) => {
      try {
        ...
        // 或者日期範圍應該透過其他方式設定
        const data = await sdk.accounting.realizedGainsAndLosesSummary(account);
        ...
        //         const startDateFromItem = new Date(item.startDate);
        //         const endDateFromItem = new Date(item.endDate);
        //         return startDateFromItem >= startDateObj && endDateFromItem <= endDateObj;
        //       })
        //     : data.data
        // };

        const response = `API Response\n\`\`\`json\n${JSON.stringify(data, null, 2)}\n\`\`\`\n\nField Description\n\`\`\`json\n${JSON.stringify(realizedPnLSumReference, null, 2)}\n\`\`\``;
        ...
        return {
          content: [
            {
              type: "text",
              text: `已實現損益彙總查詢時發生錯誤: ${error || "未知錯誤"}`,
            },
          ],
          isError: true,
        };
